...
@numba.jit(nopython=config._numba_nopython, cache=config._numba_cache, parallel=config._numba_parallel, nogil=config._numba_nogil)
...
@numba.jit(nopython=config._numba_nopython, cache=config._numba_cache, parallel=config._numba_parallel, nogil=config._numba_nogil)
def _update_bests_helper(fitness, swarm, pbest, pbest_fitness, gbest, gbest_fitness, minimize):
    '''
    ...
    :return: The updated personal best, the updated personal best fitness, the updated global best, and the updated global best fitness.
    '''
    if minimize:
...
class GBestPSO(PSO):
    def __init__(self, problem, swarm_size, dim, max_iter, minimize=True, w=0.7298, c1=1.49618, c2=1.49618):
        '''
        A class for the global best particle swarm optimization algorithm. This particle swarm optimization algorithm uses an inertia weight, cognitive parameter, and social parameter.
    ...
    def _update_velocity(self):
        ...
        '''
        self.velocity = _update_velocity_helper(self.w, self.c1, self.c2, self.swarm, self.velocity, self.pbest, self.gbest)
